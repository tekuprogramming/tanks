...
class Game(arcade.Window):
        ...
        # fields
        self.left_pressed = False # left arrow button isn't pressed
        self.right_pressed = False # right arrow button isn't pressed
        self.up_pressed = False
        self.down_pressed = False

    def on_draw(self): # responsible for everything drawn in the window
        arcade.draw_texture_rectangle(WIDTH / 2, HEIGHT / 2, WIDTH, HEIGHT, self.bg)
        self.green_tank.draw()

    def on_key_press(self, key, modifiers): # watches for when the button is pressed
        if key == arcade.key.LEFT: # if we press the left arrow button
    ...
    def on_key_release(self, key, modifiers): # watches for when the button is released
        if key == arcade.key.LEFT and not self.right_pressed: # stops the movement when we release the button
